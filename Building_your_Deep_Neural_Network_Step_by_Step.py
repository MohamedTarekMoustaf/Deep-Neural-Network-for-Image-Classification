...
def L_model_forward(X, parameters):
    ...
    for l in range(1, L): # iterate "L-1" times excluding the output layers whose index is "L".
        A_prev = A 
        #(≈ 2 lines of code)
        # A, cache = ...
        # caches ...
...
def compute_cost(AL, Y):
    ...
    # YOUR CODE STARTS HERE
     #soln_1
    cost=-1*np.mean((np.multiply(Y,np.log(AL))+np.multiply(1-Y,np.log(1-AL))),axis=1,keepdims=True)
    
     #soln_2
    #AL,Y=np.squeeze(AL),np.squeeze(Y) # convert to 1D to make np.dot produce the sum dot product.
    #cost = float(-1/m*(np.dot(np.log(AL),Y)+np.dot(np.log(1-AL),1-Y)))
    # YOUR CODE ENDS HERE
